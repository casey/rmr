...
/*
- update remotely without having to use the macbook air
- make as few changes to the mux as possible

- get window size to be correct
- figure out that random encoder crash
- maybe use async pbos for better performance
- make a release compile and put in Applications
- try out twitch release library

whitelisted client - http://www.twitch.tv/kraken/oauth2/clients/35621 (romrador)

option for connectivity: https://www.facebook.com/danielstroem
*/
...
struct twitch_t::data_t {
  ...
  void check(const char* step, TTV_ErrorCode result) {
    if (TTV_FAILED(result)) {
      rmr << step;
      rmr << string(step) + " failed: " + TTV_ErrorToString(result);
      rmr.exit(0);
    }
  }
  ...
  static void callback(TTV_ErrorCode result, void* p) {
    ...
    if (TTV_FAILED(result)) {
      rmr << string("twitch.callback: failed: ") + TTV_ErrorToString(result);
      rmr.exit(0);
    }
  ...
  data_t(const string& un, const string& pw, const string& cs) : username(un), password(pw), client_secret(cs) {
    ...
    auth_params.userName     = username     .c_str();
    auth_params.password     = password     .c_str();
    auth_params.clientSecret = client_secret.c_str();

    audio_params.size                   = sizeof(TTV_AudioParams);
    audio_params.audioEnabled           = false;
    audio_params.enableMicCapture       = false;
    audio_params.enablePlaybackCapture  = false;
  ...
  void ingest_done() {
    ...
    for (int i = 0; i < ingest_list.ingestCount; i++) {
      ...
      if (name.find("Los Angeles") != string::npos) {
        server = i;
      }
    ...
    if (server == -1) {
      rmr << "twitch.ingest_done: couldn't find default ingest server";
      rmr.exit(0);
    }
  ...
  void poll() {
    ...
    if (state == state_t::idle && width != 0 && height != 0) {
      ...
      video_params.maxKbps                = 256;
      video_params.targetFps              = 30;
      video_params.encodingCpuUsage       = TTV_ECU_MEDIUM;
      video_params.disableAdaptiveBitrate = true;
...
twitch_t::twitch_t(const string& username, const string& password, const string& client_secret)
: __(*new data_t(username, password, client_secret)) { }
...
const string& twitch_t::status()                { return __.status;                 }
