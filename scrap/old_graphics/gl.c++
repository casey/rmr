...
gl_t::gl_t(win_t& w) : __(*new data_t(w)) {
  new_vao("");
  bind_vao("");
...
_ gl_t::prepare(sym_t program) {
  ...
  static u32 installed_program = 0;

  __.start_preparing();

  if (installed_program != p) {
    glUseProgram(p);
    installed_program = p;
  }
...
_ gl_t::value(sym_t s, texture_unit_t u) {
  ...
  else            glUniform1i(var.location, u.value);
  var.ready = true;
  return *this;
}
...
bool gl_t::first() { return __.w.first(); }
...
gl_t& gl_t::new_prg(sym_t s) {
  ...
  try {
    string prg_name = s;
    string src;
    if (rmr.startswith(prg_name, "st_")) {
      prg_name = prg_name.erase(0, 3);
      string st_src = rmr.slurp(rmr.cfg("base") + "rsc/shadertoy/" + prg_name + ".glsl");
      string st_tmp = rmr.slurp(rmr.cfg("base") + "src/shadertoy.glsl");
      src = rmr.replace(st_tmp, "SHADERTOY_BODY_GOES_HERE_LOL_BBQ", st_src);
    } else {
      src = rmr.slurp(rmr.cfg("base") + "src/" + s + ".glsl");
    }
    ...
    for(auto path : {"/common.glsl"}) {
      auto key   = path;
      auto value = rmr.slurp(rmr.cfg("base") + "src" +  path);
      glNamedStringARB(GL_SHADER_INCLUDE_ARB, -1, key, -1, value.c_str());
    }
    ...
    auto shader = [&](string& src, uint type){
      ...
      if (!gl_get_shader<GL_COMPILE_STATUS>(s)) {
        ...
        fail("gl.new_prg: compilation error: "s + log);
      }
    };
    ...
    auto link_and_validate = [&](){
      ...
      if (!gl_get_program<GL_VALIDATE_STATUS>(p)) {
        fail("gl.new_prg: validation error");
      }
    };
...
const string& gl_t::enum_name(uint e) {
  ...
  if (table.empty()) {
    table[GL_INVALID_ENUM                 ] = "GL_INVALID_ENUM";
    table[GL_INVALID_VALUE                ] = "GL_INVALID_VALUE";
    table[GL_INVALID_OPERATION            ] = "GL_INVALID_OPERATION";
    table[GL_INVALID_FRAMEBUFFER_OPERATION] = "GL_INVALID_FRAMEBUFFER_OPERATION";
    table[GL_OUT_OF_MEMORY                ] = "GL_OUT_OF_MEMORY";
    table[GL_NO_ERROR                     ] = "GL_NO_ERROR";
  }

  if (!table.count(e)) {
    rmr.fatal() << "gl.enum_name: unknown enum:" << e;
  }
