...
//_ node_t::rotate_x(float angle) { transform().rotate_x(angle); return *this; }
//_ node_t::rotate_y(float angle) { transform().rotate_y(angle); return *this; }
//_ node_t::rotate_z(float angle) { transform().rotate_z(angle); return *this; }
...
debug_node_t& debug_node_t::spawn(const sym_t& name) {
  return node_t::template spawn<debug_node_t>(name);
}

debug_node_t& debug_node_t::clone(const sym_t& name) {
  return node_t::template clone<debug_node_t>(name);
}

void debug_node_t::gl(gl_t& gl) {
}

void debug_node_t::render(gl_t& gl, const frame_t<node_t*>& nodes) {
  for (auto p : nodes) {
    ...
    .value(SYM(color), vec3::hash(p).extend(1.0))
    .ready()
    .points()
    .draw_arrays(0, 1)
    .done()
