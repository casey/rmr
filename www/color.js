...
define(function(require) {
  var rmr = require('array');

  rmr.color = function () {
    if (arguments.length === 0) {
      var color = rmr.vec4(1.0, 1.0, 1.0, 1.0);
    } else if (arguments.length === 3 || arguments.length === 4) {
      var color = rmr.color.fromArray(arguments);
    } else if (typeof arguments[0] === 'string') {
      var s = arguments[0];
      if (s === "random") {
        var color = rmr.vec4().randomize().a(1);
      } else if (s === "random_alpha") {
        var color = rmr.vec4().randomize();
      } else {
        var color = rmr.color.fromString(arguments[0]);
      }
    } else if (arguments[0] && arguments[0].length >= 3) {
      var color = rmr.color.fromArray(arguments[0]);
    } else {
      rmr.error('color: bad arguments:', arguments);
    }
    ...
    if (isNaN(color[3]) || arguments.length === 3) {
      color[3] = 1;
    }

    rmr.color.verify(color);

    return color;
  };

  rmr.color.fromArray = function(a) {
    var color = rmr.vec4();

    if (a.length !== 3 && a.length !== 4) {
      rmr.error('rmr.color.fromArray: array has bad length:', a);
    }
  ...
  rmr.color.fromString = function(s) {
    s = s.toLowerCase();

    if (s === "random") {
      return rmr.vec4().randomize().a(1);
    }

    if (s === "random_alpha") {
      return rmr.vec4().randomize();
    }

    if (rmr.color.names.hasOwnProperty(s)) {
      return rmr.color.fromString(rmr.color.names[s]);
    }

    var stripped = rmr.color.stripName(s);
    if (rmr.color.strippedNames.hasOwnProperty(stripped)) {
      return rmr.color.fromString(rmr.color.strippedNames[stripped]);
    }
    ...
    if (s[0] === '#' || s[0] === 'x') {
      ...
      switch (s.length) {
        case 5: // #RGBA
          color[4] = parseInt(s.slice(4, 5), 16) / 15;
        case 4: // #RGB
          color[0] = parseInt(s.slice(1, 2), 16) / 15;
          color[1] = parseInt(s.slice(2, 3), 16) / 15;
          color[2] = parseInt(s.slice(3, 4), 16) / 15;
          break;
        case 9: // #RRGGBBAA
          color[4] = parseInt(s.slice(7, 9), 16) / 255;
        case 7: // #RRGGBB
          color[0] = parseInt(s.slice(1, 3), 16) / 255;
          color[1] = parseInt(s.slice(3, 5), 16) / 255;
          color[2] = parseInt(s.slice(5, 7), 16) / 255;
          break;
        default:
          console.error('parseStringColor: bad color string:', s);
      }
    } else {
      console.error('parseStringColor: bad colorString:', s);
    }

    return color;
  };

  rmr.color.verify = function(color) {
    if (color.length !== 4) {
      rmr.error('node.verifyColor: bad length:', color.length);
    }

    var r = color[0];
    if (!(r >= 0 && r <= 1)) {
      rmr.error('node.verifyColor: bad red component:', r);
    }

    var g = color[1];
    if (!(g >= 0 && g <= 1)) {
      rmr.error('node.verifyColor: bad green component:', g);
    }

    var b = color[2];
    if (!(b >= 0 && b <= 1)) {
      rmr.error('node.verifyColor: bad blue component:', b);
    }

    var a = color[3];
    if (!(a >= 0 && a <= 1)) {
      rmr.error('node.verifyColor: bad alpha component:', a);
    }
  };
  ...
  rmr.color.strippedNames = {};

  rmr.color.stripName = function (s) { return s.replace(/[^a-z]/g, ''); };

  _(rmr.color.names).keys().each(function (original) {
    var stripped = rmr.color.stripName(original);
    rmr.color.strippedNames[stripped] = rmr.color.names[original];
  });

  // todo: pre-parse colors
